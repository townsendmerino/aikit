...
3. **Int8 HNSW** — ✅ **DONE.** Gate first (the roadmap's prerequisite):
   `TestHNSW_int8RecallGate` (dequantize proxy) showed recall@10 Δ0.0000 vs f32 on
   real embeddings — risk retired. Then productionized behind `Config.Int8`: the
   graph stores `bq []int8` + `scales` (¼ vector memory), `simIDs` uses the new
   exported `linalg.DotI8` (AVX2/SDOT) for node-node dots, and `sim` quantizes the
   query once per search via a prepared `queryVec` threaded through
   `greedyClosest`/`searchLayer` (the f32 path is behaviorally unchanged). Persisted
   format bumped to **v3** (int8-mode byte + int8 codes/scales); the int8 blob is
   ~half the f32 at dim 64 (more at higher dim). Validated: `TestHNSW_int8_real`
   recall@10 Δ0.0000 vs f32; int8 round-trip is query-identical; `FuzzLoadHNSW`
   covers the v3 int8 path; -race clean; cgo-free + windows cross-build. The
   ¼-memory win now extends to the log-scale index (the large-repo embedded case).
