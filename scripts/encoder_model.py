#!/usr/bin/env python3
"""encoder_model.py — thin wrapper around nomic-ai/CodeRankEmbed for the
...
class CodeRankReranker:
    ...
    def __init__(
        ...
        *,
        ...
        sys.stderr.write(
            f"[encoder] loading {MODEL_ID} on {self.device} "
    ...
    def prewarm(self, strings: Iterable[str]) -> None:
        ...
        if not missing:
            return
        sys.stderr.write(f"[encoder] prewarming {len(missing)} unique strings...\n")
        sys.stderr.flush()
